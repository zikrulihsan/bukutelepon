...
export function ContactCard({ contact }: ContactCardProps) {
  ...
  function handleSave(e: React.MouseEvent) {
    e.stopPropagation();
    const result = toggleSaved(contact.id);
    setSaved(result);
  }
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
      <div className="p-4 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            onClick={handleSave}
            className="flex-shrink-0 w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center active:scale-95 transition-transform"
            title={saved ? "Hapus dari tersimpan" : "Simpan kontak"}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className={`h-5 w-5 ${saved ? "text-primary-700" : "text-gray-400"}`}
          ...
          <div className="border-t border-gray-100 pt-3">
            ...
            <div className="flex gap-2 mb-3">
              <a
                ...
                className="flex-1 h-10 rounded-xl bg-primary-700 flex items-center justify-center gap-2 active:scale-[0.98] transition-transform"
                onClick={(e) => e.stopPropagation()}
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-white" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                </svg>
                <span className="text-xs font-semibold text-white">Telepon</span>
              </a>
              <a
                href={formatWhatsAppUrl(contact.phone)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 h-10 rounded-xl bg-[#25D366] flex items-center justify-center gap-2 active:scale-[0.98] transition-transform"
                onClick={(e) => e.stopPropagation()}
              >
