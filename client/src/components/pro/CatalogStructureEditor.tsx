...
const navigationOptions: Array<{ value: CatalogNavigationStyle; title: string; copy: string; icon: string }> = [
  { value: "COMPACT_SLIDER", title: "Compact slider", copy: "Baris ringkas dengan thumbnail", icon: "▤" },
  { value: "POSTER_SLIDER", title: "Poster slider", copy: "Poster visual besar yang digeser", icon: "▥" },
  { value: "GRID", title: "Grid card", copy: "Card tersusun tanpa slider", icon: "▦" },
];
...
export function CatalogStructureEditor({ business, userId }: { business: ManagedBusiness; userId: string }) {
  ...
  const [navigationStyle, setNavigationStyle] = useState<CatalogNavigationStyle>(business.catalogNavigationStyle ?? "COMPACT_SLIDER");
  ...
  useEffect(() => {
    ...
    setNavigationStyle(business.catalogNavigationStyle ?? "COMPACT_SLIDER");
  }, [business.catalogAccent, business.catalogNavigationStyle, business.catalogPreset, business.catalogTheme, business.defaultItemLayout]);

  const refresh = () => queryClient.invalidateQueries({ queryKey: ["pro", "business"] });

  const savePresentation = useMutation({
    mutationFn: async () => (await apiClient.put("/pro/business/presentation", { catalogPreset: preset, defaultItemLayout: defaultLayout, catalogTheme: theme, catalogAccent: accent, catalogNavigationStyle: navigationStyle })).data,
  ...
  return (
    <section className="mt-5 space-y-5">
      ...
      <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm sm:p-7">
        ...
        <div className="mt-6 border-t border-gray-100 pt-5"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-primary-700">Jelajahi katalog</p><h3 className="mt-1 text-base font-bold text-gray-900">Pilih model navigasi grup</h3><p className="mt-1 text-xs text-gray-500">Mengatur tampilan pintasan kategori di bagian atas katalog.</p></div>
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          {navigationOptions.map((option) => <button key={option.value} type="button" onClick={() => setNavigationStyle(option.value)} className={`rounded-2xl border p-3 text-left transition ${navigationStyle === option.value ? "border-primary-500 bg-primary-50 ring-2 ring-primary-100" : "border-gray-200 hover:border-gray-300"}`}><span className="text-2xl text-primary-700">{option.icon}</span><strong className="mt-2 block text-sm text-gray-900">{option.title}</strong><span className="mt-1 block text-[10px] leading-4 text-gray-500">{option.copy}</span></button>)}
        </div>
      ...
      {draft && <div className="fixed inset-0 z-[90] flex items-end justify-center sm:items-center sm:p-5" role="dialog" aria-modal="true"><button type="button" className="absolute inset-0 bg-gray-950/55 backdrop-blur-sm" onClick={() => setDraft(null)} aria-label="Tutup" /><form onSubmit={submit} className="relative max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-gray-900">{draft.id ? "Edit section" : "Tambah section"}</h2><p className="text-xs text-gray-500">Komponen ini akan tampil di katalog publik.</p></div><button type="button" onClick={() => setDraft(null)} className="grid h-10 w-10 place-items-center rounded-full bg-gray-100 text-gray-500"><HiXMark className="h-5 w-5" /></button></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className={labelClass}>Jenis section<select value={draft.type} onChange={(event) => setDraft({ ...draft, type: event.target.value as CatalogSectionType })} className={inputClass}><option value="ITEM_GROUP">Grup item</option><option value="BANNER">Banner besar</option><option value="PROMOTION">Promosi</option><option value="ACTIVITY">Aktivitas</option><option value="INFORMATION">Informasi</option></select></label><label className={labelClass}>Judul *<input required minLength={2} maxLength={160} value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} className={inputClass} placeholder={draft.type === "ITEM_GROUP" ? "Coffee" : draft.type === "BANNER" ? "Temukan favorit di Ramya" : "Promo sarapan"} /></label></div>
